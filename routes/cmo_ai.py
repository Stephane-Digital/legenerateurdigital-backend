...
from services.ai.cmo_ai import generate_cmo_dispatch, generate_cmo_strategy
...
class CmoDispatchRequest(BaseModel):
    objective: str
...
def _estimate_strategy_tokens(payload: CmoStrategyRequest) -> int:
    text = " ".join(
        [
...
def _estimate_dispatch_tokens(payload: CmoDispatchRequest) -> int:
    text = " ".join(
        [
            str(payload.objective or ""),
            str(payload.blocker or ""),
            str(payload.target_module or ""),
            str(payload.niche or ""),
            str(payload.audience or ""),
            str(payload.offer or ""),
            str(payload.current_situation or ""),
...
def cmo_dispatch(
    payload: CmoDispatchRequest,
    db: Session = Depends(get_db),
    current_user: Any = Depends(get_current_user),
):
    uid = _user_id(current_user)
    amount = _estimate_dispatch_tokens(payload)
    ...
    try:
        result = generate_cmo_dispatch(
            objective=payload.objective,
            blocker=payload.blocker or "",
            target_module=payload.target_module or "",
...
def cmo_strategy(
    ...
    uid = _user_id(current_user)
    amount = _estimate_strategy_tokens(payload)
...
def cmo_health():
    return {
        "status": "ok",
        "module": "CMO IA V5",
        "mode": "dispatch_and_strategy",
        "endpoints": ["/cmo-ai/dispatch", "/cmo-ai/strategy"],
    }
