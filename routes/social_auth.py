...
def get_auth_url(provider: str):
    ...
    if provider in ("facebook", "instagram"):
        raise HTTPException(
            status_code=410,
            detail="Ce flux OAuth est obsolète pour Facebook/Instagram. Utilise /social-connections."
        )
...
def oauth_callback(
    ...
    if provider in ("facebook", "instagram"):
        raise HTTPException(
            status_code=410,
            detail="Ce callback OAuth est obsolète pour Facebook/Instagram. Utilise /social-connections."
        )
